...
// POST /contact-messages — public, create a new message
...
// GET /contact-messages/:id — admin only
...
// PUT /contact-messages/:id — mark read/unread (admin only)
router.put("/contact-messages/:id", requireAdmin, async (req, res) => {
  try {
    const id = parseInt(req.params.id as string);
    const { isRead } = req.body;
    const [msg] = await db.update(contactMessagesTable)
      .set({ isRead })
...
// POST /contact-messages/:id/reply — send email reply (admin only)
    ...
    if (RESEND_API_KEY) {
      try {
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: "Pearlis Jewellery <concierge@pearlis.com>",
            to: [msg.email],
            subject: `Re: ${msg.subject}`,
            html: `
              <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
                <div style="border-bottom: 1px solid #c9a84c; padding-bottom: 20px; margin-bottom: 30px;">
                  <h1 style="font-size: 28px; letter-spacing: 4px; color: #1a1a1a; margin: 0;">PEARLIS</h1>
                  <p style="color: #888; font-size: 11px; letter-spacing: 3px; margin: 4px 0 0;">FINE JEWELLERY</p>
                </div>
                <p style="color: #666; font-size: 14px;">Dear ${msg.firstName},</p>
                <div style="background: #faf9f7; border-left: 3px solid #c9a84c; padding: 20px; margin: 20px 0; font-size: 15px; line-height: 1.7;">
                  ${replyText.replace(/\n/g, "<br>")}
                </div>
                <p style="color: #666; font-size: 14px;">Warm regards,<br><strong>Pearlis Concierge Team</strong></p>
                <div style="border-top: 1px solid #eee; margin-top: 30px; padding-top: 20px; font-size: 11px; color: #aaa; letter-spacing: 1px;">
                  <p>This reply is in response to your inquiry: "${msg.subject}"</p>
                  <p>© ${new Date().getFullYear()} Pearlis Fine Jewellery. All rights reserved.</p>
                </div>
              </div>
            `,
          }),
        ...
        req.log.error(emailErr, "Failed to send email via Resend");
      }
    }
...
// POST /contact-messages/bulk-reply — send bulk reply (admin only)
router.post("/contact-messages/bulk-reply", requireAdmin, async (req, res) => {
  try {
    const { ids, replyText } = req.body;
    if (!ids || !Array.isArray(ids) || !replyText) {
      res.status(400).json({ error: "ids array and replyText are required" });
      return;
    }
    ...
    for (const id of ids) {
      ...
      await db.update(contactMessagesTable)
        .set({ replied: true, repliedAt: new Date(), adminReply: replyText, isRead: true })
        .where(eq(contactMessagesTable.id, id));
